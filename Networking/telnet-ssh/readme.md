...
## Introducción
Telnet y SSH son protocolos utilizados para administrar en forma remota dispositivos como swtiches, routers, servidores. La diferencia, es que SSH envía el tráfico cifrado, mientras telnet lo hace en texto plano.
...
End
```
No se utilizó el comnado `service password-encryption`, por lo que la contraseña VTY aparece en texto plano. Para la contraseña en modo privilegiado se utilizó el comando `enable secret [password]`, para que se en la salida del `show running-config` se vea cifrada. 
...
Switch#
```
Tipeamos algunos comendos, por ejemplo `show running-config`, pasamos las diferentes pantallas para ver toda la configuración y salimos. Detenemos la captura en Wiresark y pasamos a analizar qué vemos. 
[IMAGEN WIRESHARK]
...
Se observan los tres mensajes (`SYN, SYN-ACK, ACK`) y luego comienza el tráfico telnet. Nos paramos con el mousse y seleccionamos cualquiera de los paquetes telnet y hacemos click derecho sobre él.
...
Con esto verificamos que el comando `enable secret` (y también `service password-encryption`) solo cifra las contraseñas en el archivo de configuración local. 
...
End
```
Se debe configurar un hostname y dominio al equipo. Configuramos un usuario y contraseña, que serán las credenciales que se utilizarán para acceder al equipo. 
