...
Utilizaremos como máquina atacante un Kali Linux y como víctima a la conocida Metasploitable2, una VM especialmente configurada para ser atacada y realizar diferentes pruebas de pentesting. 
...
Con estos datos, es posible consultar fuentes públicas (por ejemplo, `cve.org` o NVD) para determinar si existen vulnerabilidades conocidas asociadas a esas versiones.
...
```
![NMAP](images/nmapsc.png)
![NMAP](images/nmapsc2.png)
...
**PUERTO 22**
  ...
  - Protocolo SSH 2.0
  - Claves públicas del servidor: DSA 1024 bits y RSA 2048 bits
...
Con la opción `-O` Nmap puede estimar qué sistema operativo se encuentra en ejecución, incluso sin tener acceso al sistema. Esta información puede utilizarse para identificar vulnerabilidades específicas del sistema operativo y verificar compatibilidad de exploits u otras herramientas de ataque.
```bash
nmap -O 192.168.0.200
```
![NMAP](images/nmapo.png)

## Conclusión
Durante este laboratorio comprobamos la utilidad de Nmap en la fase de reconocimiento, realizando diferentes pruebas para identificar múltiples servicios expuestos, y obtener información detallada de esos servicios.
